#已增加stop,straight,left,slow,speed
...
class XYDataset(torch.utils.data.Dataset):
    ...
    def refresh(self):
        ...
        for category in self.categories:
            ...
            for image_path in glob.glob(os.path.join(self.directory, category, '*.jpg')):
                x, y, stop, straight, left, slow, speed = self._parse(image_path)
                self.annotations += [{
    ...
    def __getitem__(self, idx):
        ...
        return image, ann['category_index'], torch.Tensor([x, y, stop, straight, left, slow, speed])
    ...
    def _parse(self, path):
        ...
        slow = items[6]
        speed = items[7]
        return int(x), int(y), int(stop), int(straight), int(left), int(slow), int(speed)
        
    def save_entry(self, category, image, count, x, y, stop, straight, left, slow, speed):
        category_dir = os.path.join(self.directory, category)
        if not os.path.exists(category_dir):
            subprocess.call(['mkdir', '-p', category_dir])
            
        filename = '%d_%d_%d_%d_%d_%d_%d_%d_%s.jpg' % (count, x, y,stop,straight,left,slow,speed, str(uuid.uuid1()))
    ...
    def save_auto(self, category, image, count):
        ...
        filename = '%d_0_0_0_0_0_0_0_%s.jpg' % (count, str(uuid.uuid1()))
